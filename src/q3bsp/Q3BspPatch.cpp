...
void Q3BspBiquadPatch::Render(bool renderLightmaps)
{    
    glVertexPointer(3, GL_FLOAT, sizeof(Q3BspVertexLump), &m_vertices[0].position);

    glClientActiveTextureARB(GL_TEXTURE0_ARB);
    glTexCoordPointer(2, GL_FLOAT, sizeof(Q3BspVertexLump), &m_vertices[0].texcoord[0]);

    glClientActiveTextureARB(GL_TEXTURE1_ARB);
    glTexCoordPointer(2, GL_FLOAT, sizeof(Q3BspVertexLump), &m_vertices[0].texcoord[1]);

    // disable tex0 - render only lightmap texture
    if(renderLightmaps)
    {
